...
from .models import Pharmacy, SalesDocument, SalesLine, Visit, StockoutObservation, Product, Zone, FormDefinition, Client, FormFieldDefinition, FormSubmission, FormAnswer
...
class FormListView(LoginRequiredMixin, ListView):
    ...
    template_name = "analytics/form_list.html"
    context_object_name = "forms"

    def get_queryset(self):
        # Filter filters by active and client.
        # Handle case where user might not have a rep_profile or client relation easily
        client = None
        if hasattr(self.request.user, 'rep_profile'):
            # rep_profile is a RelatedManager (ForeignKey)
            rep = self.request.user.rep_profile.first()
            if rep:
                client = rep.client
        
        if not client:
            # Fallback for admin or other users if they have a direct client FK or assume first
             client = Client.objects.first() # Simplification for demo
...
class FormFillView(LoginRequiredMixin, TemplateView):
    template_name = "analytics/form_fill.html"
    ...
    def get_context_data(self, **kwargs):
        ...
        # Determine current visit/pharmacy context. 
        # For simplicity in this demo, we'll fetch the first active pharmacy of the client
        # Ideally this would come from a selected "Visit" or "Pharmacy" in the URL or previous step.
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Extract Visit/Pharmacy Context
        pharmacy_id = request.POST.get('pharmacy_id')
        
        # Create a Visit on the fly (Simplified for this flow)
        # In a real app, you might select an existing visit.
        ...
        with transaction.atomic():
            ...
            visit = Visit.objects.create(
                ...
                rep=request.user.rep_profile.first() if hasattr(request.user, 'rep_profile') else None, # Fallback
                pharmacy=pharmacy,
        ...
        return redirect('analytics:form_list')

class SubmissionListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    model = FormSubmission
    template_name = "analytics/submission_list.html"
...
class SubmissionDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    model = FormSubmission
    template_name = "analytics/submission_detail.html"
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Fetch answers sorted by the original field order
        context['answers'] = (
            self.object.answers
